...
plot(lasso_fit, xvar="lambda", col=rainbow(p), label=TRUE, main="Plot 3: Effect of the Tuning Parameter on Parameter Estimates", cex.main=0.8)
...
MSE <- lasso_cv_fit_train$cvm[which_lambda_lasso] #0
...
This plot presents the cross-validation scores for cytological characteristics. [For each value of lambda, the mean MSE across the k folds is plotted alogn with error bars which cover the mean plus or minus one standard error.0

To choose an appropriate value for the turning parameter of LASSO, 10-fold cross-validation is used. This allows the optimal turning parameter and mean MSE to be calculated, which are 0.01963041 and 0, respectively. [UPDATE: The regression coefficients obtained by performing the LASSO with the optimal turning parameter are shown in the table below. - Optimal or optimal train?!]

```{r lasso_coefficients, include=FALSE}

# Create table
lasso_coefficients <- tribble(~"", ~"Intercept", ~"CT", ~"CSh", ~"MA", ~"BN", ~"BC", ~"NN", "Estimates", 1.12, 0.35, 0.52, 0.15, 0.60, 0.25, 0.17)
...
When using our train and test data sets for out-of-sample validation, the test error is 0%, [Something is wrong with code]
